...
export default class CadastroReserva extends Component {
	...
	componentDidMount() {
		...
		setTimeout(() => {		
			...
			if (this.props.location.state !== undefined && this.state.usuario.restauranteId !== undefined) {
				fetch(`https://scenic-lake-clark-26080.herokuapp.com/api/reserva/${this.props.location.state.id}/${this.state.usuario.restauranteId}`, requestInfo)
					.then(response => {
						if (response.ok) {
	...
	envia(event) {
		...
		fetch('https://scenic-lake-clark-26080.herokuapp.com/api/reservas', requestInfo)
			.then(response => {
