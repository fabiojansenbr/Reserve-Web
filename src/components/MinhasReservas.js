...
export default class MinhasReservas extends Component{
	...
	atualizaReservas() {
		...
		setTimeout(() => {
			fetch(`https://scenic-lake-clark-26080.herokuapp.com/api/minhasreservas/${this.state.usuario.clienteId}`, requestInfo)
				.then(response => {
					if (response.ok) {
		...
		swal({
			...
			if (result.value) {
				swal(
					...
					fetch(`https://scenic-lake-clark-26080.herokuapp.com/api/reservas/${id}`, requestInfo)
						.then(response => {
							if (response.ok) {
								return response.json();
							} else {
